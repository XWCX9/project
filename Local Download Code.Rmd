...
### Extracting tables of weights for unrelated individual instruments:
```{r}
un_bmi_wei <- extract_outcome_data(snps = un_bmi_instrument$SNP, outcomes = 'ukb-b-19953')
un_bmi_wei <- subset(un_bmi_wei, select = c("SNP", "beta.outcome"))
save(un_bmi_wei, file = "un_bmi_wei.RData")
```

```{r}
un_hei_wei <- extract_outcome_data(snps = un_hei_instrument$SNP, outcomes = 'ieu-a-89')
un_hei_wei <- subset(un_hei_wei, select = c("SNP", "beta.outcome"))
save(un_hei_wei, file = "un_hei_wei.RData")
```
...
### The weights:
```{r}
un_ed_wei <- extract_outcome_data(snps = un_ed_instrument$SNP, outcomes = 'ieu-a-1239')
un_ed_wei <- subset(un_ed_wei, select = c("SNP", "beta.outcome"))
save(un_ed_wei, file = "un_ed_wei.RData")
```
...
### Their weights:
```{r}
wf_hei_wei <- extract_outcome_data(wf_hei_instrument$SNP, outcomes = 'ieu-b-4814')
wf_hei_wei <- subset(wf_hei_wei, select = c("SNP", "beta.outcome"))
save(wf_hei_wei, file = "wf_hei_wei.RData")
```

```{r}
wf_bmi_wei <- extract_outcome_data(wf_bmi_instrument$SNP, outcomes = 'ieu-b-4816') # cane from a small instrument
wf_bmi_wei <- subset(wf_bmi_wei, select = c("SNP", "beta.outcome"))
save(wf_bmi_wei, file = "wf_bmi_wei.RData")
```
...
### Extracting the weights for this instrument:
```{r}
wf_rev_ed_wei <- extract_outcome_data(snps = wf_rev_ed_instrument$SNP, outcomes = 'ieu-b-4836')
wf_rev_ed_wei <- subset(wf_rev_ed_wei, select = c("SNP", "beta.outcome"))
save(wf_rev_ed_wei, file = "wf_rev_ed_wei.RData")
```
